...
import useStationTable from '@src/pages/StormManagement/StormInformation/table/stationColumns'
import { StationType } from '@src/constants/common'
...
export interface StationInfoDialogRef {
  open: (id: number, type: StationType) => void
  close: () => void
}
...
const StationInfoDialog = forwardRef<StationInfoDialogRef, StationInfoDialogProps>((props, ref) => {
  ...
  const handleOpen = (id: number, type: StationType) => {
    setType(type)
    setOpen(true)
  }
  ...
  return (
    <DialogContainer open={open} onClose={handleClose} maxWidth={'lg'}>
      <DialogHeader onClose={handleClose}>Thông tin trạm</DialogHeader>
      <DialogBody>
        <Typography color="primary.main" fontSize={24} fontWeight={700}>
          Danh sách trạm {type ? `${type}`.toUpperCase() : ''}
        </Typography>
        <Box
          sx={{
            height: 1,
